...
WorkloadEstimator::WorkloadEstimator(HomaConfigDepot *homaConfig)
    : cdfFromFile(), cbfFromFile(), remainSizeCdf(), remainSizeCbf(), cbfLastCapBytesFromFile(), avgSizeFromFile(0.0), rxCdfComputed(), sxCdfComputed(), homaConfig(homaConfig)
{
    const char *workloadType = homaConfig->workloadType;
    std::string base_path = "./config/homa-size-distributions";
    if (workloadType != NULL)
    {
        std::string distFileName;
        if (strcmp(workloadType, "DCTCP") == 0)
        {
            distFileName = base_path + std::string("/DCTCP_MsgSizeDist.txt");
        }
        else if (strcmp(workloadType, "TEST_DIST") == 0)
        {
            distFileName = base_path + std::string(
                                           "/TestDistribution.txt");
        }
        else if (strcmp(workloadType, "FACEBOOK_KEY_VALUE") == 0)
        {
            distFileName = base_path + std::string("/FacebookKeyValueMsgSizeDist.txt");
        }
        else if (strcmp(workloadType, "FACEBOOK_WEB_SERVER_INTRACLUSTER") == 0)
        {
            distFileName = base_path + std::string("/Facebook_WebServerDist_IntraCluster.txt");
        }
        else if (strcmp(workloadType, "FACEBOOK_CACHE_FOLLOWER_INTRACLUSTER") == 0)
        {
            distFileName = base_path + std::string("/Facebook_CacheFollowerDist_IntraCluster.txt");
        }
        else if (strcmp(workloadType, "FACEBOOK_HADOOP_ALL") == 0)
        {
            distFileName = base_path + std::string("/Facebook_HadoopDist_All.txt");
        }
        else if (strcmp(workloadType, "FABRICATED_HEAVY_MIDDLE") == 0)
        {
            distFileName = base_path + std::string("/Fabricated_Heavy_Middle.txt");
        }
        else if (strcmp(workloadType, "FABRICATED_HEAVY_HEAD") == 0)
        {
            distFileName = base_path + std::string("/Fabricated_Heavy_Head.txt");
        }
        else if (strcmp(workloadType, "GOOGLE_SEARCH_RPC") == 0)
        {
            distFileName = base_path + std::string("/Google_SearchRPC.txt");
        }
        else if (strcmp(workloadType, "GOOGLE_ALL_RPC") == 0)
        {
            distFileName = base_path + std::string("/Google_AllRPC.txt");
        }
        ...
        {
            throw std::runtime_error("Not a valid workload type: " + std::string(workloadType));
        }
        ...
        {
            std::cout << "Failed to open file: " << distFileName << std::endl;
            exit(1);
        }
...
void WorkloadEstimator::getRemainSizeCdfCbf(CdfVector &cdf, uint32_t cbfCapMsgSize,
                                            uint32_t boostTailBytesPrio)
{
    ...
    hdr_homa homaUnschedPkt = hdr_homa();
    homaUnschedPkt.pktType_var() = PktType::UNSCHED_DATA;
    uint32_t unschedPerPkt = MAX_ETHERNET_PAYLOAD - IP_HEADER_SIZE -
                             UDP_HEADER_SIZE - homaUnschedPkt.headerSize();
    // uint32_t unschedHdr = hdrs + homaUnschedPkt.headerSize();

    hdr_homa homaSchedPkt = hdr_homa();
    homaSchedPkt.pktType_var() = PktType::SCHED_DATA;
    uint32_t schedPerPkt = MAX_ETHERNET_PAYLOAD - IP_HEADER_SIZE -
    ...
    for (auto &sizeProbPair : cdf)
    {
        ...
        while (sizeRemain)
        {
            ...
            if (sizeRemain == size)
            {
                ...
                pktOnWire = hdr_homa::getBytesOnWire(pkt, PktType::REQUEST);
            }
            else if (pktedSize < totalUnsched)
            {
                pkt = std::min(totalUnsched - pktedSize, unschedPerPkt);
                // pktOnWire = pkt+unschedHdr;
                pktOnWire = hdr_homa::getBytesOnWire(pkt, PktType::UNSCHED_DATA);
            }
            else
            {
                pkt = std::min(schedPerPkt, sizeRemain);
                // pktOnWire = pkt+schedHdr;
                pktOnWire = hdr_homa::getBytesOnWire(pkt, PktType::SCHED_DATA);
                // allGrantBytes += grantHdr;
                allGrantBytes += hdr_homa::getBytesOnWire(0, PktType::GRANT);
            }
...
void WorkloadEstimator::getCbfFromCdf(CdfVector &cdf, uint32_t cbfCapMsgSize,
                                      uint32_t boostTailBytesPrio)
{
    cbfFromFile.clear();
    ...
    uint32_t req = homaConfig->defaultReqBytes;
    uint32_t unsched = homaConfig->defaultUnschedBytes;
    uint32_t maxEthFrame = MAX_ETHERNET_PAYLOAD +
                           ETHERNET_PREAMBLE_SIZE + ETHERNET_HEADER_SIZE + INTER_PKT_GAP_SIZE;
    ...
    hdr_homa homaSchedPkt = hdr_homa();
    homaUnschedPkt.pktType_var() = PktType::SCHED_DATA;
    uint32_t schedPerPkt = MAX_ETHERNET_PAYLOAD - IP_HEADER_SIZE -
                           UDP_HEADER_SIZE - homaSchedPkt.headerSize();
    ...
    for (auto &sizeProbPair : cdfFromFile)
    {
        ...
        if (size < req)
        {
            reqOnWire = hdr_homa::getBytesOnWire(std::min(req, size),
                                                 PktType::REQUEST);
        }
        else if (size < req + unsched)
        {
            reqOnWire = hdr_homa::getBytesOnWire(req, PktType::REQUEST);
            unschedOnWire = hdr_homa::getBytesOnWire(std::min(size - req, unsched),
                                                     PktType::UNSCHED_DATA);
        }
        else
        {
            reqOnWire = hdr_homa::getBytesOnWire(req, PktType::REQUEST);
            unschedOnWire = hdr_homa::getBytesOnWire(unsched,
                                                     PktType::UNSCHED_DATA);
            schedOnWire = hdr_homa::getBytesOnWire(size - req - unsched,
                                                   PktType::SCHED_DATA);
            uint32_t numGrants = (uint32_t)ceil((1.0 * schedOnWire) / maxEthFrame);
            grantsOnWire = numGrants * hdr_homa::getBytesOnWire(0, PktType::GRANT);
        }
...
void WorkloadEstimator::recomputeRxWorkload(uint32_t msgSize, double timeMsgArrived)
{
}

void WorkloadEstimator::recomputeSxWorkload(uint32_t msgSize, double timeMsgSent)
{
}
