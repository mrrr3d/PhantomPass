...
void TimerHandler::sched(double delay)
{
	...
	{
		fprintf(stderr, "Couldn't schedule timer - already scheduled\n");
		abort();
	}
	_sched(delay);
...
void TimerHandler::resched(double delay)
{
	...
	{
		_cancel();
		// printf("resched > cancel() \n");
	}
	// printf("resched > not cancel() \n");
	_sched(delay);
	status_ = TIMER_PENDING;
	// printf("TimerHandler::resched END \n");
}

void TimerHandler::handle(Event *e)
{
	// printf("TimerHandler::handle %d at %f\n", status_, Scheduler::instance().clock());
	if (status_ != TIMER_PENDING) // sanity check
		abort();
	// printf("TimerHandler::handle after abort\n");
