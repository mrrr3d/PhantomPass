...
{
    slog::log6(debug_, -1, "XPassDropTail::updateTokenBucket()");
    double now = Scheduler::instance().clock();
    ...
    // port to SIRD: avoid oveflowing int during first pkt due to 10s elapsed time
    elapsed_time = min(elapsed_time, 0.1);
...
{
    // std::cout << "data_q_ byteLength: " << data_q_->byteLength() << std::endl;
    if (p == NULL)
    {
        return;
    }
    ...
    else
    {
        ...
        {
            assert(0); // SIRD xpass port - should never happen with infinite buffers
...
void XPassDropTail::attach_node(Node *node)
{
    if (node == nullptr)
    {
