...
for file_name in onlyfiles:
    ...
    for rep_num in range(REP_NUM):
        ...
        output_dir_name = 'REQUEST_SIZE/'
        command = "scavetool x --type v --filter \"module(**.server[*].app[*]) AND " \
                  "\\\"requestSize:vector\\\"\" -o {} -F CSV-S {}\n".format(OUTPUT_FILE_DIRECTORY+output_dir_name+output_file_name, vector_file_name)
        print(command)
        f.write('echo \"{}\"\n'.format(output_dir_name))
        f.write(command)

        output_dir_name = 'REQUEST_RECEIVED/'
        command = "scavetool x --type v --filter \"module(**.server[*].app[*]) AND " \
                  "\\\"requestReceivedRequesterID:vector\\\"\" -o {} -F CSV-S {}\n".format(OUTPUT_FILE_DIRECTORY+output_dir_name+output_file_name, vector_file_name)
        f.write('echo \"{}\"\n'.format(output_dir_name))
        f.write(command)

        output_dir_name = 'REQUEST_SOURCE/'
        command = "scavetool x --type v --filter \"module(**.server[*].app[*]) AND " \
                  "\\\"requestSource:vector\\\"\" -o {} -F CSV-S {}\n".format(OUTPUT_FILE_DIRECTORY+output_dir_name+output_file_name, vector_file_name)
        f.write('echo \"{}\"\n'.format(output_dir_name))
        f.write(command)

        output_dir_name = 'REQUEST_DURATION/'
        command = "scavetool x --type v --filter \"module(**.server[*].app[*]) AND " \
                  "\\\"requestDuration:vector\\\"\" -o {} -F CSV-S {}\n".format(OUTPUT_FILE_DIRECTORY+output_dir_name+output_file_name, vector_file_name)
        f.write('echo \"{}\"\n'.format(output_dir_name))
        f.write(command)
        ...
        output_dir_name = 'SENDER_REQUEST_SIZE/'
        command = "scavetool x --type v --filter \"module(**.server[*].app[*]) AND " \
                  "\\\"senderRequestSize:vector\\\"\" -o {} -F CSV-S {}\n".format(OUTPUT_FILE_DIRECTORY+output_dir_name+output_file_name, vector_file_name)
        f.write('echo \"{}\"\n'.format(output_dir_name))
        f.write(command)
        ...
        output_dir_name = 'SAMPLED_QUEUE_LENGTH/'
        command = "scavetool x --type v --filter \"module(**.**.eth[*].mac.queue*) AND " \
                  "\\\"queueLenBytesSampled:vector\\\"\" -o {} -F CSV-S {}\n".format(OUTPUT_FILE_DIRECTORY+output_dir_name+output_file_name, vector_file_name)
        print(command)
        f.write('echo \"{}\"\n'.format(output_dir_name))
        f.write(command)

        output_dir_name = 'SAMPLED_DEPARTED_BYTES/'
        command = "scavetool x --type v --filter \"module(**.**.eth[*].mac.queue*) AND " \
                  "\\\"byteDepartures:vector\\\"\" -o {} -F CSV-S {}\n".format(OUTPUT_FILE_DIRECTORY+output_dir_name+output_file_name, vector_file_name)
        print(command)
        f.write('echo \"{}\"\n'.format(output_dir_name))
        f.write(command)
