...
BASE_SPINE_NUM = 4
BASE_AGG_NUM = 9
SERVERS_UNDER_EACH_RACK = 16
TOTAL_DC_SERVER_NUM = BASE_AGG_NUM * SERVERS_UNDER_EACH_RACK

BASE_DIR="extracted_results"
'''
    Directories used for processing the extracted results
'''
directory = f'./{BASE_DIR}/REQUEST_SENT/'
directory2 = f'./{BASE_DIR}/FLOW_STARTED/'
directory3 = f'./{BASE_DIR}/FLOW_ENDED/'
directory4 = f'./{BASE_DIR}/REPLY_LENGTH_ASKED/'
directory5 = f'./{BASE_DIR}{BASE_DIR}/FLOW_ENDED_QUERY_ID/'
                                ...
                                for incast_flow_size in INCAST_FLOW_SIZE:
                                    for incast_scale in NUM_REQUESTS_PER_BURST:
                                        ...
                                        for rep_num in range(REP_NUM):
                                            ...
                                            print(f"TOTAL_DC_SERVER_NUM {TOTAL_DC_SERVER_NUM}")
                                            print(f"Opening {directory + file_name} to find flow info")
                                                ...
                                                for row in csv_reader:
                                                    ...
                                                    for k in range(0, len(row), 2):
                                                        ...
                                                        try:
                                                            ...
                                                            print(f"Server: {server_idx}")
                                                            # EXPECTATION. Each inner loop maps one server_idx to one request sent
                                                            # The first elemnt of row1 (server) corresnponts to the first element of
                                                            # each line of data (aka, the first column).
                                                            # FLow id has nothing to do with this and it should be unique based on
                                                            # results of original code.
                                                            # print(f" ROW1[k]: {row1[k]}") # one per server
                                                            server_flow_list[server_idx].append(Flow(rep_num=rep_num, ID=flow_id, record_time=time,
                                                                                                     start_time=time))
                                                        except:
                                                            continue
                                            pprint(server_flow_list)
                                            print(f"Opening {directory2 + file_name} to find flow info")
                                            # This is the flowStartedSignal. It captures the moment when the firt packet of the 
                                            # response arrives at the client (not what I want)
                                            ...
                                            print(f"============================ server_flow_hash_list =================== {len(server_flow_hash_list)}")
                                            print(f"Opening {directory3 + file_name} to find flow info")
                                        ...
                                        print(f"--------- fct_cdf_info ----------- {len(fct_cdf_info)}")
                                        pprint(fct_cdf_info)
...
if len(all_flow_completions[0]) != len(x_values):
    print(f"all_flow_completions[0] {all_flow_completions[0]}    x_values: {x_values}")
