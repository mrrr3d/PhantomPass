...
simsignal_t DCTcpGenericServerApp::requestReceivedSignal = registerSignal("requestReceivedSignal");
simsignal_t DCTcpGenericServerApp::requestSourceSignal = registerSignal("requestSourceSignal");
simsignal_t DCTcpGenericServerApp::requestDurationSignal = registerSignal("requestDurationSignal");
simsignal_t DCTcpGenericServerApp::requestSizeSignal = registerSignal("requestSizeSignal");
...
void DCTcpGenericServerApp::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL)
    {
        ...
        if (delay.dbl() > 0.0000001)
            throw cRuntimeError("Processing delay is underisbale");
        maxMsgDelay = 0;
    ...
    else if (stage == INITSTAGE_APPLICATION_LAYER)
    {
        ...
        // // std::cout << "SERVER LOCAL ADDR: " << localAddress << std::endl; // nothing
...
void DCTcpGenericServerApp::handleMessage(cMessage *msg)
{
    ...
    // std::cout << "SEPEHR: handleMessage called and msg is: " << msg << std::endl;
    ...
    else if (msg->getKind() == TCP_I_PEER_CLOSED)
    {
        ...
        // std::cout << "Peer closed itself and sent FIN. Closing after " << maxMsgDelay << "s" << std::endl;
    ...
    else if (msg->getKind() == TCP_I_DATA || msg->getKind() == TCP_I_URGENT_DATA)
    {
        ...
        // std::cout << "SEPEHR: packet received " << packet << std::endl;
        ...
        while (mchunk)
        {
            ...
            if (mchunk->getLength() + mchunk->getOffset() == main_chunk->getChunkLength())
            {
                ...
                // std::cout << "SEPEHR: appmsg is " << appmsg << std::endl;
                requester_id = appmsg->getRequesterID();
                requested_time = appmsg->getRequested_time();
                long src_id = appmsg->getQuery_id();
                // std::cout << "Request is thoroughly received, lets send the response. From " << src_id << " Req size: " << main_chunk->getChunkLength().get() / 8 << " requsted time: " << requested_time << std::endl;

                emit(DCTcpGenericServerApp::requestReceivedSignal, requester_id);
                emit(DCTcpGenericServerApp::requestSourceSignal, src_id);
                emit(DCTcpGenericServerApp::requestDurationSignal, (simTime() - requested_time));
                emit(DCTcpGenericServerApp::requestSizeSignal, main_chunk->getChunkLength().get() / 8);
                ...
                // std::cout << "SEPEHR: requested bytes is: " << requestedBytes << std::endl;
                simtime_t msgDelay = appmsg->getReplyDelay();
                if (msgDelay.dbl() > 0.0000001)
                    throw cRuntimeError("msgDelay is undesirable");
                ...
                if (requestedBytes > B(0))
                {
                    ...
                    // std::cout << "SEPEHR: setting requester id for the reply. Requester ID is: " << requester_id << std::endl;
            ...
            // std::cout << "This is not a complete request, keep waiting to receive all of it!" << std::endl;
            // }
        ...
        if (doClose)
        {
            ...
            // std::cout << "The other application set ServerClose to True. Closing after " << maxMsgDelay << "s" << std::endl;
            sendOrSchedule(request, maxMsgDelay);
        }
    }
