...
void AugmentedEtherMac::initialize(int stage)
{
    ...
    // std::cout << "AugmentedEtherMac::initialize()" << endl;
...
void AugmentedEtherMac::handleMessageWhenUp(cMessage *msg)
{
    ...
    MacAddress addr = getMacAddress();
    // std::cout << "AugmentedEtherMac::handleMessageWhenUp. getMacAddress: " << addr << endl;
    printState();
...
void AugmentedEtherMac::handleUpperPacket(Packet *packet)
{
    ...
    // std::cout << "Received " << packet << " from upper layer." << endl;
    ...
    // std::cout << "local mac address: " << address << " dest: " << frame->getDest() << endl;
...
void AugmentedEtherMac::processJamSignalFromNetwork(EthernetSignal *msg)
{
    ...
    // std::cout << "Received " << msg << " from network.\n";
...
{
    // std::cout << "Received " << signal << " from network.\n";

    if (!connected || disabled)
    {
        EV_WARN << (!connected ? "Interface is not connected" : "MAC is disabled") << " -- dropping msg " << signal << endl;
        if (typeid(*signal) == typeid(EthernetSignal))
        { // do not count JAM and IFG packets
...
{
    throw std::runtime_error("Wtf kind of protocol is this?");
...
{
    throw std::runtime_error("I don't want any pauses at the link layer");
...
{
    // std::cout << "AugmentedEtherMac::processReceivedDataFrame" << endl;
    // statistics
...
void AugmentedEtherMac::fillIFGIfInBurst()
{
    ...
    if (currentTxFrame && endIFGMsg->isScheduled() && (transmitState == WAIT_IFG_STATE) && (simTime() == lastTxFinishTime) && (simTime() == endIFGMsg->getSendingTime()) && (framesSentInBurst > 0) && (framesSentInBurst < curEtherDescr->maxFramesInBurst) && (bytesSentInBurst + INTERFRAME_GAP_BITS + PREAMBLE_BYTES + SFD_BYTES + calculatePaddedFrameLength(currentTxFrame) <= curEtherDescr->maxBytesInBurst))
    {
...
void AugmentedEtherMac::add_on_the_way_packet(b packet_length)
{
