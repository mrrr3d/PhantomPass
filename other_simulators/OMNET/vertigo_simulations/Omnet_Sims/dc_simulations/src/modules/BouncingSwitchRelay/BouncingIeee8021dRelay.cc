...
void BouncingIeee8021dRelay::initialize(int stage)
{
    ...
    // std::cout << "BouncingIeee8021dRelay::initialize()" << std::endl;
...
{
    // std::cout << "BouncingIeee8021dRelay::registerAddress: " << mac << std::endl;
...
void BouncingIeee8021dRelay::handleUpperPacket(Packet *packet)
{
    ...
    else if (frame->getDest().isBroadcast())
    { // broadcast address
        broadcast(packet, -1);
    }
    else
...
void BouncingIeee8021dRelay::handleAndDispatchFrame(Packet *packet)
{
    ...
    // BPDU Handling
    if (isStpAware && (frame->getDest() == MacAddress::STP_MULTICAST_ADDRESS || frame->getDest() == bridgeAddress) && arrivalPortData->getRole() != Ieee8021dInterfaceData::DISABLED && isBpdu(packet, frame))
    {
    ...
    {
        // std::cout << "Destination address = " << frame->getDest() << " frame: " << frame << endl;
...
InterfaceEntry *BouncingIeee8021dRelay::find_interface_to_fw_randomly_power_of_n(Packet *packet, bool consider_servers)
{
    if (true)
    {
        throw cRuntimeError("I think this is not supposed to happen with ECMP");
    }
