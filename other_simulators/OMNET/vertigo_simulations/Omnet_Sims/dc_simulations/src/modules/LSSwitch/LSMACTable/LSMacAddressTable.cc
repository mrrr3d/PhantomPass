...
void LSMacAddressTable::readAddressTable(const char *fileName)
{
    ...
    // std::cout << getParentModule()->getIndex() << " Current working directory: " << cwd << " opening file: " << fileName << std::endl;
    ...
    for (int lineno = 0; (line = fgetline(fp)) != nullptr; delete[] line)
    {
        ...
        std::list<simtime_t> insertionTime;
        interfaceList.push_back(interfaceId);
        // std::cout << "pushing interface " << interfaceId << " to interfaceList of size: " << interfaceList.size() << std::endl;
...
void LSMacAddressTable::initializeTable()
{
    ...
    if (this->addressTable != nullptr)
    {                                                     // setup a WATCH on VLANID 0 if present
