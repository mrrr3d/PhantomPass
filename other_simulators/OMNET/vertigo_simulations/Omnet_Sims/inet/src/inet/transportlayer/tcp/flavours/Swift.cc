...
#define MAX_CWND_PACKET_NUMBER 666667      // borrowed from TCP. around 65536 bytes. Update: to work on >100Gbps fabrics, updateing to 1GB based on this: https://learn.microsoft.com/en-us/troubleshoot/windows-server/networking/description-tcp-features
...
simtime_t Swift::calculate_target_delay(int type) {
    ...
    if (type == ENDPOINT) {
        ...
        // std::cout << "target delay type is endpoint" << std::endl;
        target_delay = state->constant_endpoint_target_delay;
    } else if (type == FABRIC) {
        // EV << "target delay type is fabric" << endl;
        // std::cout << "target delay type is fabric" << std::endl;
        ...
        // std::cout << "alpha is " << alpha << ", beta is " << beta << ", and hop_count_to_be_echod is " << conn->hop_count_to_be_echod << std::endl;
        if (state->has_delay_scaling) {
            // EV << "delay scaling is on" << endl;
            // std::cout << "delay scaling is on" << std::endl;
    ...
    // std::cout << "target delay is " << target_delay << std::endl;
    return target_delay;
}
...
void Swift::set_final_cwnd_values(simtime_t now, uint32 prev_ecwnd) {
    ...
    // std::cout << simTime() << " cwnd in bytes is " << state->snd_cwnd << std::endl;
...
bool Swift::sendData(bool sendCommandInvoked)
{
    ...
    // std::cout << simTime() << " Swift::sendData called. " << sendCommandInvoked << std::endl;

    if (sendCommandInvoked) {
        // Reset congestion window to send new message
        // sendCommandInvoked is true only when a new message is sent
        if (state->use_custom_IW) {
                state->snd_cwnd = state->custom_IW_mult * state->snd_mss;
                fcwnd = state->custom_IW_mult;
                ecwnd = state->custom_IW_mult;
        } else {
            throw cRuntimeError("No other case should be possible.");
        }
        // std::cout << simTime() << " Restarting, CWND is set to " << state->snd_cwnd << std::endl;
    }
    //
    ...
    if (!conn->isSendQueueEmpty()) {    // do we have any data to send?
        if ((simTime() - state->time_last_data_sent) > state->rexmit_timeout) {
            ...
            if (state->increased_IW_enabled) {
                throw cRuntimeError("state->increased_IW_enabled is true. I don't know what this is and I should investigate if it happens");
            ...
            {
                throw cRuntimeError("!state->use_custom_IW. I don't want the window to start from scratch in these simulations.");
                state->snd_cwnd = state->snd_mss;
            }
            // EV << "Restarting idle connection, CWND is set to " << state->snd_cwnd << "\n";
            // std::cout << simTime() << " Restarting idle connection, CWND is set to " << state->snd_cwnd << std::endl;
        }
    }
...
void Swift::receivedDataAck(uint32 firstSeqAcked)
{
    ...
    // EV << "Before changing: ecwnd: " << ecwnd << ", fcwnd: " << fcwnd << endl;
    // std::cout << "Before changing: ecwnd: " << ecwnd << ", fcwnd: " << fcwnd << std::endl;
    ...
    if (state->dupacks >= DUPTHRESH) {    // DUPTHRESH = 3
        ...
        // std::cout << "endpoint delay is " << state->end_point_delay << " target: " << endpoint_target_delay << std::endl;
        ...
        // std::cout << "fabric delay is " << state->fabric_delay*1000.0*1000.0 << " target: " << fabric_target_delay*1000.0*1000.0 << std::endl;
    ...
    // EV << "After changing: ecwnd: " << ecwnd << ", fcwnd: " << fcwnd << endl;
    // std::cout << "After changing: ecwnd: " << ecwnd << ", fcwnd: " << fcwnd << std::endl;

    set_final_cwnd_values(now, prev_ecwnd);
    // std::cout << "After seting final values: ecwnd: " << ecwnd << ", fcwnd: " << fcwnd << std::endl;
